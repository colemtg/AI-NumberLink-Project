...
let getPuzzles: FileInput list = 
  ...
  let filePath = "AI-NumberLink-Project/testCases2.txt"
...
for i in getPuzzles do
  ...
  thePuzzles <- i :: thePuzzles

thetimes <- reverse thetimes
thePuzzles <- reverse thePuzzles
let filePath = "AI-NumberLink-Project/analysis_results2.txt"
let fullPath = Path.Combine(baseDirectory'.FullName, filePath)
...
let printNumbersToFile fileName =
  ...
  for puzzle in thePuzzles do
    let (num, theString) = puzzle
    fprintf file "%s" ("Puzzle: " + string(num) + " " + theString)
    fprintf file "\n"
